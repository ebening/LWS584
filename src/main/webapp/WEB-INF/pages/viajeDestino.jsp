...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header"><%= etiqueta.VIAJE_DESTINO %></h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
					...
					<div class="panel panel-default">
						<div style="height: 54px;" class="panel-heading">
							<button style="float: right;" type="button"
								...
								data-target="#modal"><%= etiqueta.AGREGAR %> <%= etiqueta.VIAJE_DESTINO_LBL %></button>
						</div>
						<!-- /.panel-heading -->
						<div class="panel-body">
							<div class="dataTable_wrapper">
								<table class="table table-striped table-bordered table-hover"
									...
									<tbody>
										<c:forEach items="${viajeDestinoList}" var="list">
											<tr class="odd gradeX">
												<td><c:out value="${list.idViajeDestino}" /></td>
												<td><c:out value="${list.descripcion}" /></td>
												<td class="center">
													<button title="<%= etiqueta.ELIMINAR %>"
														onclick="eliminar(${list.idViajeDestino})" style="height: 22px;"
														type="button" class="btn btn-xs btn-danger">
														<span class="glyphicon glyphicon-trash"></span>
													</button>

													<button title="<%= etiqueta.EDITAR %>"
														onclick="editar(${list.idViajeDestino})" style="height: 22px;"
	...
	<div class="modal fade" id="modal" role="dialog">
		<div class="modal-dialog">
			...
			<form:form id="form" cssClass="form-horizontal"
				modelAttribute="viajeDestino" method="post" action="saveViajeDestino">
					...
					<div class="modal-body">
						...
						<div class="form-group">
							<div class="control-label col-xs-3">
								<form:label path=""><%=etiqueta.DESCRIPCION%></form:label>
							</div>
							<div class="col-xs-6">
								<form:hidden id="idViajeDestino" path="idViajeDestino" value="" />
						...
						<div class="form-group">
							<div class="control-label col-xs-3">
								<label for="esSolicitanteB"><%= etiqueta.ES_VIAJE_INTERNACIONAL %></label>
							</div>
							<div class="checkbox col-xs-6">
								<form:checkbox id="esViajeInternacionalB" path="esViajeInternacionalB" value="" />
							</div>
						</div>
  ...
  <!-- DataTables JavaScript -->
	...
	e_titulo = '<%= etiqueta.VIAJE_DESTINO_LBL %>';
	...
	$(document).ready(function() {
        ...
        $('#modal').on('hidden.bs.modal', function () {
        	...
        	$("#esViajeInternacionalB").prop("checked", false);
        	$("#idViajeDestino").val(0);
        });
	...
	function editar(id) {
		...
		if(id > 0){
            ...
            $.ajax({
                type: "GET",
                cache: false,
                url: "${pageContext.request.contextPath}/getViajeDestino",
                async: true,
                data: "intxnId=" + id,
                success: function(response) {
                	loading(false);
                	console.log(response.descripcion+" -> "+response.idViajeDestino);
                	$("#descripcion").val(response.descripcion);
                	$("#idViajeDestino").val(response.idViajeDestino);
                	if(response.esViajeInternacional == 1){
                  		$("#esViajeInternacionalB").prop("checked", true);
                  	}
                  	else{
                  		$("#esViajeInternacionalB").prop("checked", false);
                  	} 
		...
		function eliminar(id){
			...
			console.log(id);
			$("#eliminarButton").prop("href","deleteViajeDestino?idViajeDestino="+id);
